...
def clean_Non_CHINESE_PUN(line):
    return ' '.join(re.findall(ur"[\u4e00-\u9fa5|]+", line)).encode('utf-8')
...
def Fast_T2S(lines, Keep_CP = True): # 并行版，将繁体转换成简体
    pool = Pool(cpu_count())
    if Keep_CP:
        results = pool.map(T2S, lines)
    else:
        results = pool.map(T2S_NCP, lines)
    pool.close()
    pool.join()
    return results

def Fast_S2T(lines, Keep_CP = True): # 并行版，将简体转换成繁体
    pool = Pool(cpu_count())
    if Keep_CP:
        results = pool.map(S2T, lines)
    else:
        results = pool.map(S2T_NCP, lines)
    pool.close()
    pool.join()
    return results

def TXT_T2S(path_to_file, KCP = True):
    output_path = path_to_file +'.simple'
    with open(path_to_file,'rb') as input_f:
        with open(output_path,'wb') as output_f:
            output_f.writelines('\n'.join(Fast_T2S(input_f.readlines(),KCP)))
    return 'File in: %s ' % output_path

def TXT_S2T(path_to_file, KCP = True):
    output_path = path_to_file +'.traditional'
    with open(path_to_file,'rb') as input_f:
        with open(output_path,'wb') as output_f:
            output_f.writelines('\n'.join(Fast_S2T(input_f.readlines(),KCP)))
    return 'File in: %s ' % output_path
